...
function App() {
  ...
  const [noData, setNoData] = useState("Sem informações no momento!");
  const [searchTerm, setSearchTerm] = useState("");
  const [weatherData, setWeatherData] = useState([]);
  const [city, setCity] = useState("Cidade não informada!");
  ...
  const getWeather = async (location) => {
    ...
    try {
      ...
      &lang=pt_br&appid=${API_KEY}&units=metric&cnt=5&exclude=hourly,minutely`)
      let data = await res.json()
      if(data.cod != 200) {
        setNoData("Localização desconhecida")
        return
      }
      setWeatherData(data)
  ...
  return (
    <div className="container">
      <div className="blur" style={{top: '-10px', right: '0'}}></div>
      ...
      <div className="content">
        <div className="form-container">
          ...
          <div className="search">
            <h2>O aplicativo de clima que você precisa!</h2>
            <hr />
            <form className="search-bar" noValidate onSubmit={handleSubmit}>
              <input type="text" placeholder="Digite uma cidade" onChange={handleChange} required />
        ...
        <div className="info-container">
          ...
          {weatherData.length === 0 ? 
            ...
            <>
              ...
              <h1 className="title">Mais sobre {city}</h1>
